...
namespace _2CSClientWin_sample.Suppliers
{
    ...
    /// <remarks>
    /// DataGridView は自動結線の対象外（FxPrefixOfGridView は Web Forms 専用）。
    /// 一覧は DataGridView に DataTable を BindingSource 経由でバインドし、
    /// ［追加］／［削除］は通常のボタン（btn 接頭辞で結線）で行う。
    /// グリッド上の編集は DataTable の RowState に乗るので、まとめて CUD できる。
    ///
    /// デスクトップ アプリなので Session は不要＝編集中の DataTable はフィールドで保持する。
    /// </remarks>
    public partial class SuppliersFormB : SuppliersBaseForm
    {
        ...
        protected void UOC_btnMain2_Click(RcFxEventArgs rcFxEventArgs)
        {
            ...
            if (returnValue.ErrorFlag)
            {
                // ★ 2CS は業務例外で自動ロールバックされない＝取り消すなら明示的に呼ぶ
                SuppliersLayerB.RollbackAndClose();
